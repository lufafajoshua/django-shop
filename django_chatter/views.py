...
from user_profiles.models import User, Profile, UserAgent
import random
...
def get_chat_url(request):
	...
	'''
@login_required
def seller_chat(request, product_id):#Get the seller object from the related product object
	user = get_user_model().objects.get(username=request.user)
	product = Product.objects.get(pk=product_id)#Filter the products with the given product id
	seller = product.seller.user
	print(seller)
	
	#agents = get_user_model().objects.filter(user_type=2)
	#print(available_agents)	
	#target_user =  	get_user_model().objects.filter(user_type=2).filter(id=seller.id) #get the seller user object via the get_user_model object		
	target_user =seller
	print(target_user)	
	if (user == target_user):
		room_id = create_room([user]) 
